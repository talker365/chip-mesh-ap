    ...
    <?php
      ...
      foreach($xml->children() as $projects) { ?>
        <div data-role="popup" id="<?php echo "projectPopup" . $projects->id; ?>">
          ...
          <h3> Prerequisites </h3>
          <?php if ( count($projects->prerequisites->children()) > 0 ) { ?>
            <div style="padding-left: 25px;">
              <dl>
                <?php foreach($projects->prerequisites->children() as $category) {
                  if ( count($category->children()) >0 ) {
                    echo "<dt>" . $category['name'] . " - " . $category->hasChildren .  "</dt>";
          ...
          <?php } else { ?>
            <div style="padding-left: 25px;"> None </div>
          <?php } ?>
